...
2. store a program to perform multiplication
3. store a secret integer to be used in the computation
4. compute the multiplication program with the stored secret and another computation time secret
"""
...
async def main():
    ...
    program_name = "multiplication_program"
    ...
    # Create a secret named "input_number" with a value, ex: 20
    new_secret = nillion.NadaValues(
        {
            "input_number": nillion.SecretInteger(20),
        }
    )

    # Set the input party for the secret
    # The party name needs to match the party name that is storing "input_number" in the program
    party_name = "Party1"
    ...
    # Add a computation time secret for the multiplication factor
    computation_time_secrets = nillion.NadaValues({"factor": nillion.SecretInteger(5)})
    ...
    print(f"The multiplication computation was sent to the network. compute_id: {compute_id}")
    while True:
